...
export function QuotationPdfButton({ quotation, onSuccess }: QuotationPdfButtonProps) {
  ...
  const generateQuotationPdf = async (email: boolean = false, docLanguage: 'en' | 'ja' = 'en'): Promise<Blob | null> => {
    ...
    try {
      ...
      const formattedQuotationId = `JPDR-${quotation?.id?.toString().padStart(4, '0') || 'N/A'}`
      ...
      quotationDate.textContent = `${quotationT.quotationDate} ${quotation?.created_at ? 
        new Date(quotation.created_at).toLocaleDateString(docLanguage === 'ja' ? 'ja-JP' : 'en-US') : 
      ...
      expiry.textContent = `${quotationT.expiryDate} ${expiryDate.toLocaleDateString(docLanguage === 'ja' ? 'ja-JP' : 'en-US')}`
      expiry.style.margin = '0 0 5px 0'
      expiry.style.fontSize = '13px'
      
      const validFor = document.createElement('p')
      validFor.textContent = `${quotationT.validFor} ${validDays} ${quotationT.days}`
      validFor.style.margin = '0'
      validFor.style.fontWeight = 'normal'
      validFor.style.fontSize = '13px'
      ...
      quotationDetails.appendChild(expiry)
      quotationDetails.appendChild(validFor)
      ...
      priceSection.style.marginBottom = '25px'
      ...
      // Service Type row with price
      const serviceTypeRow = document.createElement('div')
      serviceTypeRow.style.display = 'flex'
      serviceTypeRow.style.justifyContent = 'space-between'
      serviceTypeRow.style.marginBottom = '10px'
      
      const serviceTypeLabel = document.createElement('div')
      serviceTypeLabel.innerHTML = `<strong>${quotationT.serviceType}</strong> Charter Services (Hourly) - ${quotation?.duration || '8'} ${quotationT.hours}`
      serviceTypeLabel.style.fontSize = '13px'
      
      // Process quotation items to ensure they're in the right format
      let processedItems = [];
      try {
        if (quotation?.items) {
        ...
        console.error('Error processing quotation items:', e);
        processedItems = [];
      }
      
      // Calculate the price from items or total amount
      ...
      if (processedItems && processedItems.length > 0) {
        ...
        // Use the total from the quotation object if available
        subtotalAmount = typeof quotation?.total_amount === 'string' ? 
                        parseFloat(quotation.total_amount) || 0 : 
                        (typeof quotation?.total_amount === 'number' ? quotation.total_amount : 
                         (typeof quotation?.total === 'number' ? quotation.total : 0));
      }
      
      const serviceTypePrice = document.createElement('div')
      serviceTypePrice.textContent = formatCurrency(subtotalAmount, 'JPY')
      serviceTypePrice.style.fontSize = '13px'
      
      serviceTypeRow.appendChild(serviceTypeLabel)
      serviceTypeRow.appendChild(serviceTypePrice)
      priceDetailsContainer.appendChild(serviceTypeRow)
      
      // Vehicle Type row (no price, just description)
      ...
      vehicleTypeLabel.innerHTML = `<strong>${quotationT.vehicleType}</strong> ${quotation?.vehicle_type || 'Mercedes Benz V Class - Black Suite'}`
      vehicleTypeLabel.style.fontSize = '13px'
      
      const emptySpace = document.createElement('div')
      
      vehicleTypeRow.appendChild(vehicleTypeLabel)
      vehicleTypeRow.appendChild(emptySpace)
      ...
      subtotalLabel.textContent = quotationT.subtotal;
      subtotalLabel.style.padding = '8px 15px 8px 0';
      subtotalLabel.style.textAlign = 'right';
      subtotalLabel.style.color = '#333';
      subtotalLabel.style.fontSize = '13px';
      subtotalLabel.style.width = '50%';
      
      const subtotalValue = document.createElement('td');
      subtotalValue.textContent = formatCurrency(subtotalAmount, 'JPY');
      subtotalValue.style.padding = '8px 0';
      subtotalValue.style.textAlign = 'right';
      subtotalValue.style.color = '#333';
      subtotalValue.style.fontSize = '13px';
      subtotalValue.style.width = '50%';
      ...
      // Get discount info
      const hasDiscount = quotation?.discount_percentage && parseFloat(quotation.discount_percentage as string) > 0;
      let discountAmount = 0;
      let totalAfterDiscount = subtotalAmount;
      
      if (hasDiscount) {
        const discountPercentage = parseFloat(quotation.discount_percentage as string);
        ...
        discountLabel.textContent = `${quotationT.discount} (${discountPercentage}%)`;
        discountLabel.style.padding = '8px 15px 8px 0';
        discountLabel.style.textAlign = 'right';
        discountLabel.style.color = '#333';
        discountLabel.style.fontSize = '13px';
        
        const discountValue = document.createElement('td');
        discountValue.textContent = `-${formatCurrency(discountAmount, 'JPY')}`;
        discountValue.style.padding = '8px 0';
        discountValue.style.textAlign = 'right';
        discountValue.style.color = '#333';
      ...
      // Get tax info
      const hasTax = quotation?.tax_percentage && parseFloat(quotation.tax_percentage as string) > 0;
      let taxAmount = 0;
      let finalAmount = totalAfterDiscount;
      
      if (hasTax) {
        const taxPercentage = parseFloat(quotation.tax_percentage as string);
        ...
        taxLabel.textContent = `${quotationT.tax} (${taxPercentage}%)`;
        taxLabel.style.padding = '8px 15px 8px 0';
        taxLabel.style.textAlign = 'right';
        taxLabel.style.color = '#333';
        taxLabel.style.fontSize = '13px';
        
        const taxValue = document.createElement('td');
        taxValue.textContent = formatCurrency(taxAmount, 'JPY');
        taxValue.style.padding = '8px 0';
        taxValue.style.textAlign = 'right';
        taxValue.style.color = '#333';
      ...
      totalLabel.textContent = quotationT.total
      ...
      totalLabel.style.width = '50%'
      
      const totalAmountCell = document.createElement('td')
      // Use the calculated final amount
      totalAmountCell.textContent = formatCurrency(finalAmount, 'JPY')
      totalAmountCell.style.padding = '8px 0'
      totalAmountCell.style.textAlign = 'right'
      totalAmountCell.style.fontWeight = 'bold'
      totalAmountCell.style.color = '#333'
      totalAmountCell.style.fontSize = '14px'
      totalAmountCell.style.width = '50%'
      ...
      try {
        ...
        const pdfOptions = {
          ...
          filename: `quotation-${quotation?.id || 'details'}.pdf`,
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const pdfBlob = await generateQuotationPdf(true, emailLanguage);
      
      if (!pdfBlob) {
        throw new Error('Failed to generate quotation PDF');
      }
      
      // Make sure we have the correct MIME type for the PDF
      const pdfWithType = new Blob([pdfBlob], { type: 'application/pdf' });
      
      // Create a unique filename with the formatted quotation ID
      const formattedQuotationId = `JPDR-${quotation?.id?.toString().padStart(4, '0') || new Date().getTime()}`
      const pdfFilename = `quotation-${formattedQuotationId}.pdf`;
      ...
      if (quotation?.items && Array.isArray(quotation.items) && quotation.items.length > 0) {
        ...
        formData.append('items_data', JSON.stringify(quotation.items));
      } else if (quotation?.items && typeof quotation.items === 'string') {
        try {
          // Try to parse items if stored as JSON string
          const parsedItems = JSON.parse(quotation.items);
          if (Array.isArray(parsedItems) && parsedItems.length > 0) {
            formData.append('has_items', 'true');
            formData.append('items_data', quotation.items);
          }
        } catch (e) {
      ...
      // Add the PDF file
      formData.append('quotation_pdf', pdfWithType, pdfFilename);
