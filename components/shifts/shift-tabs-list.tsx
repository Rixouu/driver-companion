...
interface ShiftTabsListProps {
  value?: string;
  onValueChange?: (value: string) => void;
}

export function ShiftTabsList({ value, onValueChange }: ShiftTabsListProps) {
  const { t } = useI18n();
  ...
  const tabs = [
    { value: "schedule", label: t('shifts.tabs.schedule'), icon: Calendar },
    { value: "unassigned", label: t('shifts.tabs.unassigned'), icon: UserX },
    { value: "statistics", label: t('shifts.tabs.statistics'), icon: BarChart3 },
  ];
            ...
            <SelectTrigger className="w-full h-12 bg-background border-border justify-between">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <span className="text-sm font-medium truncate text-left">
                  {currentTab ? currentTab.label : t('common.menu')}
                </span>
              </div>
            </SelectTrigger>
