...
export async function POST(request: NextRequest) {
  try {
    ...
    const template = {
      ...
      type: 'email',
      category: 'quotation',
      subject: 'Invoice - Payment Required - #{{quotation_id}}',
...
Thank you for your business!`,
    ...
    if (error) {
      console.error('Error creating Invoice Email template:', error)
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Invoice Email template created successfully',
      template: data[0]
    })

  } catch (error) {
    console.error('Error in create-invoice-email API:', error)
    return NextResponse.json({ 
