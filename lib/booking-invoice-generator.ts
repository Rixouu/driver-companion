...
export async function generateBookingInvoiceHtml(
  ...
  const serviceDate = booking.date ? formatDate(booking.date) : '';
      ...
      <div style="margin-bottom: 32px;">
        ...
        <div style="background: #F9FAFB; padding: 16px; border-radius: 8px; border-left: 4px solid #3B82F6;">
          <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
            <strong>${isJapanese ? 'サービス:' : 'Service:'}</strong> ${safeEncodeText(booking.service_name || 'Service')}
          </p>
          <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
            <strong>${isJapanese ? '出発地:' : 'Pickup:'}</strong> ${safeEncodeText(booking.pickup_location || '')}
          </p>
          <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
            <strong>${isJapanese ? '到着地:' : 'Dropoff:'}</strong> ${safeEncodeText(booking.dropoff_location || '')}
          </p>
          <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
            <strong>${isJapanese ? '日付:' : 'Date:'}</strong> ${serviceDate}
          </p>
          <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
            <strong>${isJapanese ? '時間:' : 'Time:'}</strong> ${booking.time || ''}
          </p>
          ${booking.number_of_passengers ? `
            <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
              <strong>${isJapanese ? '乗客数:' : 'Passengers:'}</strong> ${booking.number_of_passengers}
            </p>
          ` : ''}
          ${previousVehicleInfo && newVehicleInfo ? `
            <p style="margin: 0 0 8px 0; font-size: 13px; color: #111827;">
              <strong>${isJapanese ? '車両:' : 'Vehicle:'}</strong> ${safeEncodeText(previousVehicleInfo.brand || '')} ${safeEncodeText(previousVehicleInfo.model || '')} (${safeEncodeText(previousVehicleInfo.name || '')})
            </p>
          ` : ''}
        </div>
      </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          ...
          <tbody>
            ...
            ${totals.regularDiscount > 0 ? `
              <tr>
                <td style="padding: 12px; font-size: 13px; color: #111827; border-bottom: 1px solid #E5E7EB;">
                  ${isJapanese ? '割引' : 'Discount'} (${booking.discount_percentage || 0}%)
                </td>
            ...
            <tr>
              <td style="padding: 12px; font-size: 13px; color: #111827; border-bottom: 1px solid #E5E7EB;">
                ${isJapanese ? '税金' : 'Tax'} (${booking.tax_percentage || 10}%)
              </td>
              <td style="padding: 12px; text-align: right; font-size: 13px; color: #111827; border-bottom: 1px solid #E5E7EB;">
      ...
      ${!isPaid ? `
        <div style="margin-bottom: 32px; padding: 16px; background: #FEF3C7; border-radius: 8px; border-left: 4px solid #F59E0B;">
          ...
          <p style="margin: 0 0 8px 0; font-size: 13px; color: #92400E;">
            ${isJapanese ? 'この請求書は未払いです。下記のリンクからお支払いをお願いいたします。' : 'This invoice is pending payment. Please complete your payment using the link below.'}
          </p>
          <p style="margin: 0; font-size: 13px; color: #92400E;">
            <strong>${isJapanese ? '支払い期限:' : 'Due Date:'}</strong> ${(() => {
