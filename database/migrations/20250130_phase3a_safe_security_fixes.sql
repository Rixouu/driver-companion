-- Phase 3A: Safe Security Fixes
-- Fix security issues without breaking existing functionality

-- =============================================
-- 1. FIX EXPOSED AUTH.USERS VIA USER_PROFILES VIEW
-- =============================================

-- Instead of dropping user_profiles, we'll modify it to be more secure
-- by removing direct auth.users access while preserving functionality

-- First, let's check if we can create a safer version
-- We'll create a new view that doesn't expose auth.users directly
...
SELECT 
  ...
  -- Add is_active logic (we'll need to get this from auth.users via a function)
  true AS is_active, -- Default to true for now
  p.created_at
FROM profiles p
WHERE p.deleted_at IS NULL;

-- Grant appropriate permissions
GRANT SELECT ON public.user_profiles_secure TO anon, authenticated;

-- =============================================
-- 2. FIX SECURITY DEFINER VIEWS (SAFE APPROACH)
-- =============================================

-- Instead of dropping views, we'll recreate them without SECURITY DEFINER
-- This preserves the existing functionality while fixing security issues
