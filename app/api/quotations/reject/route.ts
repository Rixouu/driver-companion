...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const emailPayload: any = {
      ...
      to: email,
      subject: subject || `Quotation Rejected: ${quotation.title || quotation.id}`,
      html: message || `
        ...
        <p>Your quotation has been rejected. Please find the rejected quotation attached.</p>
      `,
      attachments: [{
        filename: `rejected-quotation-${quotation.id}.html`,
        content: pdfBuffer
      }]
    };
    ...
    return NextResponse.json({ 
      message: 'Quotation rejected and email sent successfully',
      quotationId,
