...
export async function POST(request: NextRequest) {
  ...
  const timeoutId = setTimeout(() => {
    console.error('❌ [SEND REMINDER ROUTE] Request timeout after 45 seconds');
  }, 45000);

  try {
    const { quotationId, email, subject, message, includePdf = false } = await request.json();
    ...
    const emailPayload: any = {
      ...
      to: email,
      subject: subject || `Reminder: Quotation ${quotation.title || quotation.id}`,
      html: message || `
        <h2>Quotation Reminder</h2>
        <p><strong>Title:</strong> ${quotation.title}</p>
        <p><strong>Status:</strong> ${quotation.status}</p>
        <p><strong>Total Amount:</strong> $${quotation.total_amount}</p>
        <p>This is a friendly reminder about your quotation. Please review and take action.</p>
      `
    };
    ...
    if (pdfBuffer) {
      emailPayload.attachments = [{
        filename: `quotation-reminder-${quotation.id}.html`,
        content: pdfBuffer
      }];
    }
